 ...
 * @author vj002349
 */
public class ifrmLnChartVQ extends javax.swing.JInternalFrame {

    /**
     * Creates new form ifrmLnChartVQ
     */
    public ifrmLnChartVQ() {
        initComponents();
                Date fechaActual=new Date();
        SimpleDateFormat forFecha=new SimpleDateFormat("MM-dd-yyyy");
        String fecha=forFecha.format(fechaActual);
        String queryL="select VIN, TO_CHAR(VQ_TIM,'HH24:MI:SS'), row_number() over() as ROWNUM, "+
                                                         "mod_desc, col_desc from" +
                                                         "(" +
                                                         "select VIN, VQ_TIM,mod_desc, col_desc " +
                                                         "from TX_ORDEN_PROD WHERE VQ='"+fecha+"' " +
                                                         "ORDER BY VQ_TIM ASC) as d";
        
        String queryT="select row_number() over() as ROWNUM, vin, mod_desc, col_desc, anio, TO_CHAR(VQ_TIM,'dd/mm/yyyy'), TO_CHAR(VQ_TIM,'HH24:MI:SS'), vq_status, status, num_emp, tarima, contenedor, "
                + "case when id_envio = 0 then 'No' else CAST (id_envio as VARCHAR) end id_envio from" +
                                        "(" +
                                        "select vin, mod_desc, col_desc, anio,VQ_TIM, vq_status, status, num_emp, tarima, contenedor, id_envio " +
                                        "from TX_ORDEN_PROD WHERE vq='"+fecha+"' " +
                                        "ORDER BY VQ_TIM ASC " +
                                        ") as d ORDER BY ROWNUM DESC";
        String queryP="select count(*), mod_desc from TX_ORDEN_PROD WHERE vq='"+fecha+"' group by mod_desc order by count (*) desc";
        String e="Calidad-Quality";
        ...
        hChtp.start();
        queryP="select count(*), col_desc from TX_ORDEN_PROD WHERE vq='"+fecha+"' group by col_desc order by count (*) desc";
    ...
    private void initComponents() {
        ...
        setMaximizable(true);
        setResizable(true);
        setTitle("Area de Calidad");

        jLabel17.setIcon(new javax.swing.ImageIcon(getClass().getResource("/icons/VQ.png"))); // NOI18N
