 ...
 * @author vj002349
 */
public class ifrmLnChartPA extends javax.swing.JInternalFrame {

    /**
     * Creates new form ifrmLnChartPA
     */
    public ifrmLnChartPA() {
        initComponents();
        Date fechaActual=new Date();
        SimpleDateFormat forFecha=new SimpleDateFormat("MM-dd-yyyy");
        String fecha=forFecha.format(fechaActual);
        String queryL="select VIN, TO_CHAR(PA_TIM,'HH24:MI:SS'), row_number() over() as ROWNUM, "+
                                                         "mod_desc, col_desc from" +
                                                         "(" +
                                                         "select VIN, PA_TIM,mod_desc, col_desc " +
                                                         "from TX_ORDEN_PROD WHERE PA='"+fecha+"' " +
                                                         "ORDER BY PA_TIM ASC) as d ";
        String queryT="select row_number() over() as ROWNUM, vin, mod_desc, col_desc, anio, TO_CHAR(PA_TIM,'dd/mm/yyyy'), TO_CHAR(PA_TIM,'HH24:MI:SS'), pa_status, status, num_emp, tarima, contenedor, case when id_envio = 0 then 'No' else CAST (id_envio as VARCHAR) end id_envio from" +
                                        "(" +
                                        "select vin, mod_desc, col_desc, anio,PA_TIM, pa_status, status, num_emp, tarima, contenedor, id_envio " +
                                        "from TX_ORDEN_PROD WHERE pa='"+fecha+"' " +
                                        "ORDER BY PA_TIM ASC " +
                                        ") as d ORDER BY ROWNUM DESC";
        String queryP="select count(*), mod_desc from TX_ORDEN_PROD WHERE pa='"+fecha+"' group by mod_desc order by count (*) desc";
        String e="Pintura-Painting";
        ...
        hChtp.start();
        queryP="select count(*), col_desc from TX_ORDEN_PROD WHERE pa='"+fecha+"' group by col_desc order by count (*) desc";
    ...
    private void initComponents() {
        ...
        jPanel1Layout.setVerticalGroup(
            jPanel1Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
            .addComponent(jSplitPane3, javax.swing.GroupLayout.Alignment.TRAILING, javax.swing.GroupLayout.DEFAULT_SIZE, 91, Short.MAX_VALUE)
        );

        jSplitPane1.setBottomComponent(jPanel1);

        jLabel17.setIcon(new javax.swing.ImageIcon(getClass().getResource("/icons/PA.png"))); // NOI18N
