 ...
 * @author vj002349
 */
public class ifrmAddMotor extends javax.swing.JInternalFrame {
    conexionPostgres cnxO;
    /**
     * Creates new form ifrmAddMotor
     */
    public ifrmAddMotor() {
        initComponents();
        modelos();
    ...
    public void modelos(){
        conexion();
        cnxO.consulta("select distinct codigo_modelo from TX_MODELO where status='Activo'");
        try {
            while(cnxO.reg.next()){
    ...
    public void infomod(String opc){
        conexion();
        cnxO.consulta("select descripcion from TX_MODELO where status='Activo' and codigo_modelo='"+opc+"'");
        try {
            while(cnxO.reg.next()){
    ...
    public void insertar(){
        conexion();
        if(codigo.getText().length()>0&&aplica.getSelectedIndex()>0&&numCar.getText().length()>0){
            cnxO.ejecutar("INSERT INTO TX_MOTOR (motor,aplica,descripcion, status, caracteres) "
                    + "VALUES ('"+codigo.getText()+"',"
                    + "'"+aplica.getSelectedItem().toString()+"',"
                    + "'"+desc.getText()+"','"+status.getSelectedItem().toString()+"','"+numCar.getText()+"')");
