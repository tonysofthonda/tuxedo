 ...
 * @author vj002349
 */
public class afChart extends Thread{
    JPanel iframe;
                    ...
                    @Override public void handle(javafx.event.ActionEvent actionEvent) {
                        if(pc.getData().isEmpty()){
                            ...
                            try {
                                    ...
                                    while(cnx.reg.next()){
                                        pc.getData().add(new PieChart.Data("En Linea",cnx.reg.getDouble(1)));
                            ...
                            try {
                                    ...
                                    cnx.consulta("select COUNT(*) from tx_orden_prod where af_status='Pendiente-c/Motor' AND STATUS='Pendiente'");
                                    while(cnx.reg.next()){
                                        pc.getData().get(2).setPieValue(cnx.reg.getDouble(1));
                                        tmp4=cnx.reg.getInt(1);
                                    }
                            } catch (SQLException ex) {}
                            desconexion();        
                        ...
                        for (final PieChart.Data datos : pc.getData()) {
                            tmp3=Math.round((((Double) datos.getPieValue()*100)/(tmp1+tmp2+tmp4))*100.0)/100.0;
