 ...
 * @author vj002349
 */
public class ifrmSoldadura extends javax.swing.JInternalFrame {
    ...
    /**
     * Creates new form ifrmSoldadura
     * @param jDesktopPane1 
     */
    public ifrmSoldadura(JDesktopPane jDesktopPane1) {
        initComponents();
    ...
    public void lectura(){
        ...
        if(cnxO.Conec != null){
            if(bandera){
                bandera=false;
                XMLtoDB ini=new XMLtoDB(panel1,"WE", archivo);
                ini.start();
            ...
            try {
                ...
                cnxO.consulta("select we_status from TX_ORDEN_PROD WHERE VIN='"+codigo.getText().toUpperCase()+"'");
                while(cnxO.reg.next()){
                    stts=cnxO.reg.getString(1);
                }
                if("Pendiente".equals(stts)){
                    ...
                    cnxO.ejecutar("update TX_ORDEN_PROD SET we='"+fecha+"' where vin='"+codigo.getText().toUpperCase()+"'");
                    cnxO.ejecutar("update TX_ORDEN_PROD SET we_tim='"+hora+"' where vin='"+codigo.getText().toUpperCase()+"'");
                    cnxO.ejecutar("update TX_ORDEN_PROD SET we_status='Liberado' where vin='"+codigo.getText().toUpperCase()+"'");
                    cnxO.ejecutar("update TX_ORDEN_PROD SET status='Pendiente' where vin='"+codigo.getText().toUpperCase()+"'");
            ...
            bandera=true;
            wr=new writeXML();
            wr.write("WE", codigo.getText().toUpperCase());
            archivo=wr.archivo;
        }
    }
    /**
    ...
    public void pendientes(){
                if(cnxO.Conec != null){
                    cnxO.consulta("select id, VIN, MOD_DESC, ANIO, STATUS, to_char(WE,'dd-MM-yyyy'), to_char(WE_tim,'HH24:MI:SS'), WE_STATUS "
                            + "from TX_ORDEN_PROD WHERE STATUS='Pendiente' and WE_STATUS='Pendiente' order by ID DESC");
    ...
    public void liberadas(){
                if(cnxO.Conec != null){
                        cnxO.consulta("select id, VIN, MOD_DESC, ANIO, STATUS, to_char(WE,'dd-MM-yyyy'), to_char(WE_TIM,'HH24:MI:SS'), WE_STATUS "
                                + "from TX_ORDEN_PROD WHERE we_status='Liberado' and status='Pendiente' order by ID DESC");
                        //Reedefinimos un Modelo de Tabla
                                        modelo = new DefaultTableModel(){
                                            @Override
                                            public boolean isCellEditable(int rowIndex, int vColIndex) {
